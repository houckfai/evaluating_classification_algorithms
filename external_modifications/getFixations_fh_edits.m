function [cutoff,fixstart,fixend,starttime,endtime,fixdur,xmedian,ymedian,flankdataloss,fracinterped, sampleFix] = getFixations(finalweights,timestamp,xpos,ypos,missing,cutoffstd,onoffsetThresh,maxMergeDist,maxMergeTime,minFixDur)
...
% FH added: return a sample-by-sample format:
sampleFix = zeros(length(xpos), 1);

for a = 1:length(fixstart)
    idxs = fixstart(a):fixend(a);

    % FH: save to sampleFix:
    sampleFix(idxs) = 1;
