...
import os # FH added
from pathlib import Path # FH added
from scipy.signal import savgol_coeffs, filtfilt  # FH added
...
class EyegazeClassifier(object):
    ...
    def __call__(self, data, classify_isp=True, sort_events=True, source_name=None, output_dir=None):
        self.source_name = source_name  # FH added
        ...
        # FH Modification: create a sample-by-sample label timeline
        n_samples = len(data)
        time_col = np.arange(n_samples) / self.sr
        velocities = self._get_velocities(data)
        # add a zero at the beginning to match data length
        velocities = np.insert(velocities, 0, 0.0)
        sample_labels = np.array(['UNKN'] * n_samples, dtype=object)
        for ev in events:
            start_idx = int(ev['start_time'] * self.sr)
            end_idx = int(ev['end_time'] * self.sr)
            sample_labels[start_idx:end_idx] = ev['label']


        # FH Modifications to save sample-by-sample:
        if self.source_name is not None:
            source_path = Path(self.source_name)
            name_noext = source_path.stem

            if output_dir is not None:
                out_dir = Path(output_dir)
            elif "post_step1" in source_path.parts:
                # Replaces 'post_step1' with 'post_step2' and appends 'remo_results'
                parts = list(source_path.parts)
                idx = parts.index("post_step1")
                parts[idx] = "post_step2"
                # Retain path up to group folder (e.g. .../post_step2/G01/)
                out_dir = Path(*parts[:idx + 2]) / "remo_results"
            else:
                # Fallback to source file directory
                out_dir = source_path.parent / "remo_results"

            out_dir.mkdir(parents=True, exist_ok=True) # make sure folder exists to prevent FileNotFoundError
            samplewise_outfile = out_dir / f"{name_noext}_events_samplewise.csv"
        else:
            samplewise_outfile = Path("events_samplewise.csv")  # Fallback    
        
        header = "time,label,x,y"
        ...
        np.savetxt(
            ...
            delimiter=",",
            header=header,
            fmt="%s", # labels are strings
            comments=''  # to avoid '#' at the header line
        )
    ...
    def preproc(
        ...
        if savgol_length:
            ...
            # FH Edit 11.14.2025: precompute coefficients
            coeffs = savgol_coeffs(savgol_length, savgol_polyord)

            for i in ('x', 'y'):
                #data[i] = savgol_filter(data[i], savgol_length, savgol_polyord)
                # FH Edit 11.14.2025: use precomputed coefficients
                data[i] = filtfilt(coeffs, 1.0, data[i])
